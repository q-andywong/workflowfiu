...
import { ShieldCheck, ArrowRight, UserCheck, XCircle } from 'lucide-react';

const ApprovalsQueue: React.FC = () => {
    const { cases, approveCase, rejectCase, setSelectedCase, setView } = useApp();
    const [rejectionNotes, setRejectionNotes] = useState<Record<string, string>>({});
    const [activeRejectId, setActiveRejectId] = useState<string | null>(null);

    const pendingApprovals = cases.filter(c => c.status === 'PENDING_APPROVAL');
                    ...
                    {pendingApprovals.map(c => (
                        <div key={c.id} className="p-6 transition-colors hover:bg-gray-50 flex flex-col gap-4">
                            ...
                            <div className="bg-gray-100 p-4 rounded-lg flex items-center justify-between mt-2">
                                <div className="text-sm text-gray-600 font-medium">Please verify the Analyst's mitigation overrides and evidence rationale before signing off on the Case creation pipeline.</div>
                                <button 
                                    onClick={() => { setSelectedCase(c); setView('ANALYSIS'); }}
                                    className="px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-bold text-blue-600 hover:text-blue-800 shadow-sm transition-colors flex items-center gap-2"
                                >
                            ...
                            <div className="flex justify-end gap-3 mt-4 border-t border-gray-100 pt-4">
                                {activeRejectId === c.id ? (
                                    ...
                                    <>
                                        <button onClick={() => setActiveRejectId(c.id)} className="px-6 py-2 bg-white border border-red-200 text-red-600 hover:bg-red-50 text-sm font-bold rounded-lg shadow-sm transition-colors flex items-center gap-2">
                                            <XCircle className="w-4 h-4" /> REJECT
                                        </button>
                                        <button onClick={() => approveCase(c.id)} className="px-6 py-2 bg-green-600 text-white hover:bg-green-700 text-sm font-bold rounded-lg shadow-sm transition-colors flex items-center gap-2">
                                            <UserCheck className="w-4 h-4" /> SIGN-OFF & ESCALATE
                                        </button>
                                    </>
                                )}
                            </div>
